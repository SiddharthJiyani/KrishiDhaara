...
export const uploadTemperatureData = async (req, res) => {
  try {
    const { sensorNumber, units, temperature } = req.body;
...
export const getTemperatureAnalytics = async (req, res) => {
  try {
    ...
    if (!startStamp || !endStamp || !interval) {
      return res.status(400).send({
        success: false,
        message:
          "Missing required parameters: startStamp, endStamp, or interval",
      });
    }
    ...
    let result = await TemperatureData.aggregate(aggregate_pipeline);
...
export const changeTempState = async (req, res) => {
  try {
    const { sensorNumber } = req.params;
    const { state } = req.body;

    const sensorRef = ref(firebase_db, `temperature-sensors/${sensorNumber}`);
...
export const getAllTempState = async (req, res) => {
  try {
    const sensorRef = ref(firebase_db, `temperature-sensors`);
    const snapshot = await get(sensorRef);
